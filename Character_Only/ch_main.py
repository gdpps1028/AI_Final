...
def train_main():
    ...
    for epoch in range(EPOCHS): #epoch
        ...
        if val_acc > max_acc:
            ...
            torch.save(model.state_dict(), 'Character_Only/best_cnn_model.pth') # Save the best model
...
def test_main(count):
    ...
    """
    logger.info("Start loading data")
    train_dataset = utils.TrainDataset(utils.load_train_dataset()[0], utils.load_train_dataset()[1])
    ...
    criterion = nn.CrossEntropyLoss()
    model = CNN.CNN(num_unique_labels).to(device)
    model.load_state_dict(torch.load('Character_Only/best_cnn_model.pth', weights_only=True))
    test_loader = DataLoader(test_dataset, batch_size=32, shuffle=False)
    CNN.test_character(model, test_loader, criterion, device, original_labels)
...
def main(train, test_count):
    ...
    import csv
    with open('Character_Only/CNN.csv', newline='') as csvfile:
