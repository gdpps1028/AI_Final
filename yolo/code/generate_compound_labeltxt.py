...
# IDS 結構轉樹狀結構（修正不使用 reversed）
# def parse_ids(s):
#     stack = []
#     for ch in reversed(s):  # 保留 reversed
#         if ch in IDS_OPERATORS:
#             n = IDS_OPERATORS[ch]
#             if len(stack) < n:
#                 raise ValueError(f"'{ch}' 需要 {n} 個部件，但堆疊剩 {len(stack)}")
#             children = [stack.pop() for _ in range(n)]  # ❌ 去掉 [::-1]，保留原順序
#             stack.append((ch, children))
#         else:
#             stack.append(ch)
#     if len(stack) != 1:
#         raise ValueError(f"IDS 結構不合法：{s}")
#     return stack[0]
...
def assign_bbox(node):
    ...
    if not rules:
        ...
        for i, ch in enumerate(children):
            x = (i + 0.5) * w
            if isinstance(ch, str):
                result.append((ch, x, 0.5, w, 1.0))
            else:
                result += assign_bbox(ch)
        return result

    # ✅ 將規則與 children 對應順序反轉
    for ch, (x, y, w, h) in zip(reversed(children), rules):
        if isinstance(ch, str):
            result.append((ch, x, y, w, h))
        else:
            result += assign_bbox(ch)
