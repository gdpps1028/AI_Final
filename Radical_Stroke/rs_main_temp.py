...
def train_main_rs():
    ...
    # 再把 20% 當中的一半當 val，另一半當 test → 各 10%
    ...
    for epoch in range(EPOCHS): #epoch
        ...
        if val_acc > max_acc:
            ...
            torch.save(model.state_dict(), 'Radical_Stroke/best_cnn_model.pth') # Save the best model
...
def train_main_character():
    ...
    for i, rs in enumerate(rss):
        ...
        for epoch in range(EPOCHS): #epoch
            ...
            if val_acc > max_acc:
                max_acc = val_acc
                path = 'Radical_Stroke/char_model/' + rs + '.pth'
                print("Saved better model")
...
def test_main(count):
    ...
    criterion = nn.CrossEntropyLoss()
    model = CNN.CNN(num_unique_labels).to(device)
    model.load_state_dict(torch.load('Radical_Stroke/best_cnn_model.pth', weights_only=True))
    test_loader = DataLoader(test_dataset, batch_size=32, shuffle=False)
    CNN.test_rs(model, test_loader, criterion, device, original_labels)
...
def main(train, test_count):
    ...
    import csv
    with open('Radical_Stroke/CNN.csv', newline='') as csvfile:
        ...
        print("Partial entries  :", rs_correct)
        print("Accuracy         : {:.4f}".format(correct/total))
        print("Partial Accuracy : {:.4f}".format(rs_correct/total))
    return
