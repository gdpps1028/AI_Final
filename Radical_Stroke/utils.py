...
def index_to_rs(dataset : TrainDataset, index):
    return dataset.index_to_label[index]

def rs_to_index(dataset : TrainDataset, rs):
    return dataset.label_to_index[rs]
...
def load_train_dataset(labelBy: str='rs', path: str='data_new/')->Tuple[List, List]:
    images = []
    labels = []
                        
    for folder in os.listdir(path):
        if labelBy != 'rs':
            if folder.split("_")[1]+folder.split("_")[2] == labelBy:
            ...
            for image in os.listdir(path+folder):
                images.append(path+folder+"/"+image)
                labels.append(folder.split("_")[1]+folder.split("_")[2])      # labels based on rs
