...
def index_to_stroke(dataset : TrainDataset, index):
    return dataset.index_to_label[index]

def stroke_to_index(dataset : TrainDataset, stroke):
    return dataset.label_to_index[stroke]
...
def load_train_dataset(labelBy: str='stroke', path: str='data_new/')->Tuple[List, List]:
    images = []
    labels = []
                        
    for folder in os.listdir(path):
        if labelBy != 'stroke':
            if folder.split("_")[2] == labelBy:
            ...
            for image in os.listdir(path+folder):
                images.append(path+folder+"/"+image)
                labels.append(folder.split("_")[2])      # labels based on stroke
