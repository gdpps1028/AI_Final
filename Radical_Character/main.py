...
from CNN import CNN, train, validate, test_radical, test_character
...
def train_main_character():
    ...
    for i, subs in enumerate(subdirectories):
        ...
        for epoch in range(EPOCHS): #epoch
            ...
            logger.info(f"{i}, {subs}, Epoch {epoch+1}/{EPOCHS}, Train Loss: {train_loss:.4f}, Val Loss: {val_loss:.4f}, Val Acc: {val_acc:.4f}")

            if val_acc > max_acc:
                max_acc = val_acc
                path = 'char_model/' + subs + '.pth'
                print("Saved better model")
